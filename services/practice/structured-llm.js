/**
 * Generic structured-output LLM seam — duplicated from services/ai/structured-llm.js.
 *
 * services/practice is a standalone microservice with its own package.json and
 * no shared imports across service boundaries (no monorepo tooling exists in
 * this repo), so this file cannot require() the ai service's copy. This is
 * **copy #5** of LLM-provider-selection logic in this codebase, on top of the
 * four already named in that file's own header comment (resolveQuizProvider in
 * services/ai/quiz-draft.js, streamLlmResponse in services/ai/server.js, the
 * ternary before the quiz-draft call, and services/ai/structured-llm.js
 * itself). This is a deliberate, accepted cost of making practice content a
 * real standalone service rather than a module inside services/ai — see
 * HANDOFF.md and the plan this shipped from for the full rationale. Migrating
 * every copy onto one shared package is its own future unit of work.
 *
 ...
 *
 * Schemas are shape-only: OpenAI strict `json_schema` mode ignores or rejects
 * `minItems`, `maxItems`, `minimum`, `maximum` and `pattern`. Every bound must
 * live in the `validate` function passed in, which is also what drives retry.
 */
 ...
 *
 * `'practice'` → `PRACTICE`, which yields OPENROUTER_PRACTICE_MODEL,
 * GROQ_PRACTICE_TIMEOUT_MS, and so on.
 */
